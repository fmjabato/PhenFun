...
unify_filtered_data <- function(set){
	# Transfor ByModel
	set$ByModel$Signal <- as.character(set$ByModel$Signal) # Prepare container
	set$ByModel <- set$ByModel[which(set$ByModel$PvalThr == pval_thr[1]),-which(colnames(set$ByModel) %in% c("PvalThr","Ratio"))] # transform set
	invisible(lapply(seq(nrow(set$ByModel)),function(i){ # Prepare signal list format
		set$ByModel$Signal[i] <<- strsplit(as.character(set$ByModel$Signal[i]),";")
	}))

	# Transfor BySignal
	if(!is.null(set$ByAll)){
		# Prepare container
...
A_per_B <- function(MLink, model, rowsAsA = TRUE, AName, BName){
	...
	if(is.null(MLink)){
		...
		colnames(aux) <- c(AName,BName,"Prc","Model")
		return(aux)
	}
	# Transform
...
save.image("/mnt/home/users/bio_267_uma/jabato/projects/phenfunprotocol/TEST2.RData")
